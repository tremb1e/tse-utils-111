...
#define TSE_TSPI_DEFAULT_MAX_NUM_CONNECTIONS 10
...
static TSS_UUID tse_tspi_srk_uuid = TSS_UUID_SRK;
...
static void tse_tspi_to_hex(char *dst, char *src, int src_size)
{
	int x;
...
static int tse_tspi_generate_signature(char *sig, BYTE *n, uint32_t nbytes)
{
	int len, i;
	...
	tse_tspi_to_hex(sig, (char *)hash, TSE_SIG_SIZE);
...
static int
tse_tspi_deserialize(struct tspi_data *tspi_data, unsigned char *blob)
{
	int rc = 0;
...
static int tse_tspi_get_key_sig(unsigned char *sig, unsigned char *blob)
{
	...
	tse_tspi_deserialize(&tspi_data, blob);
	...
	rc = tse_tspi_generate_signature((char *)sig, n, size_n);
out:
	return rc;
}
...
struct tse_tspi_connect_ticket;

struct tse_tspi_connect_ticket {
	struct tse_tspi_connect_ticket *next;
#define TSE_TSPI_TICKET_CTX_INITIALIZED 0x00000001
	uint32_t flags;
...
static pthread_mutex_t tse_ticket_list_lock = PTHREAD_MUTEX_INITIALIZER;

static uint32_t tse_tspi_num_tickets_free;
static uint32_t tse_tspi_num_tickets_used;
static uint32_t tse_tspi_num_tickets_connected;

static struct tse_tspi_connect_ticket *ptr_to_free_ticket_list_head = NULL;
static struct tse_tspi_connect_ticket *ptr_to_used_ticket_list_head = NULL;

static int
tse_tspi_grab_ticket(struct tse_tspi_connect_ticket **ret_ticket)
{
	struct tse_tspi_connect_ticket *ticket;
	int rc = 0;

	(*ret_ticket) = NULL;
	pthread_mutex_lock(&tse_ticket_list_lock);
	ticket = ptr_to_free_ticket_list_head;
	if (!ticket) {
		struct tse_tspi_connect_ticket *tmp;
		...
		while (ticket) {
			...
			next = ticket->next;
			if (ticket->flags
			    & TSE_TSPI_TICKET_CTX_INITIALIZED) {
				pthread_mutex_unlock(&ticket->lock);
				break;
			}
			pthread_mutex_unlock(&ticket->lock);
		...
		if (!ticket) {
			...
			ticket->flags |= TSE_TSPI_TICKET_CTX_INITIALIZED;
			tse_tspi_num_tickets_connected++;
			pthread_mutex_unlock(&ticket->lock);
		}
		pthread_mutex_lock(&ticket->lock);
		...
		tse_tspi_num_tickets_free--;
		tse_tspi_num_tickets_used++;
		ticket->num_pending++;
		pthread_mutex_unlock(&ticket->lock);
	}
	pthread_mutex_unlock(&tse_ticket_list_lock);
...
static int
tse_tspi_release_ticket(struct tse_tspi_connect_ticket *ticket)
{
	int rc = 0;
...
static int
tse_tspi_encrypt(char *to, size_t *to_size, char *from, size_t from_size,
		      unsigned char *blob, int blob_type)
{
	...
	tse_tspi_deserialize(&tspi_data, blob);
	DBG_print_hex((BYTE *)&tspi_data.uuid, sizeof(TSS_UUID));
	rc = tse_tspi_grab_ticket(&ticket);
	if (rc) {
...
out:
	...
	if (ticket)
		tse_tspi_release_ticket(ticket);
	return rc;
}
...
static int
tse_tspi_decrypt(char *to, size_t *to_size, char *from, size_t from_size,
		      unsigned char *blob, int blob_type)
{
	static TSS_HPOLICY h_srk_policy = 0;
	static TSS_HKEY h_srk = 0;
...
#define TSE_KEY_MOD_PARAM_TSPI_UUID 1
static struct key_mod_param tspi_params[] = {
	{.id = TSE_KEY_MOD_PARAM_TSPI_UUID,
...
static int tse_tspi_init(char **alias)
{
	int i;
	...
	tse_tspi_num_tickets_free = 0;
	tse_tspi_num_tickets_used = 0;
	tse_tspi_num_tickets_connected = 0;
	for (i = 0; i < TSE_TSPI_DEFAULT_MAX_NUM_CONNECTIONS; i++) {
		struct tse_tspi_connect_ticket *ticket;

		ticket = malloc(sizeof(struct tse_tspi_connect_ticket));
		if (!ticket) {
			rc = -ENOMEM;
			goto out;
		}
...
static int
tse_tspi_get_params(struct key_mod_param **params, uint32_t *num_params)
{
	(*params) = tspi_params;
...
static int tse_tspi_serialize(unsigned char *blob, size_t *blob_size,
				   struct tspi_data *tspi_data)
{
	int rc = 0;
...
static int
tse_tspi_init_from_param_vals(struct tspi_data *tspi_data,
...
static int tse_tspi_get_blob(unsigned char *blob, size_t *blob_size,
				  struct key_mod_param_val *param_vals,
				  uint32_t num_param_vals)
{
	struct tspi_data tspi_data;
	...
	if ((rc = tse_tspi_init_from_param_vals(&tspi_data, param_vals,
						     num_param_vals))) {
	...
	if (blob == NULL) {
		if ((rc = tse_tspi_serialize(NULL, blob_size,
						  &tspi_data))) {
	...
	if ((rc = tse_tspi_serialize(blob, blob_size, &tspi_data))) {
		syslog(LOG_ERR, "Error serializing tspi; rc = [%d]\n", rc);
		goto out;
	}
out:
...
static int tse_tspi_destroy(unsigned char *blob)
{
	return 0;
}

#define TSE_TSPI_MAX_WAIT_FOR_END 5

static int tse_tspi_finalize(void)
{
	uint32_t retries = 0;
	struct tse_tspi_connect_ticket *ticket;
	int rc = 0;
	...
	while (ptr_to_used_ticket_list_head
	       && (retries < TSE_TSPI_MAX_WAIT_FOR_END)) {
		sleep(1);
		retries++;
	}
	if (retries == TSE_TSPI_MAX_WAIT_FOR_END) {
...
static struct tse_key_mod_ops tse_tspi_ops = {
	&tse_tspi_init,
	NULL,
	NULL,
	&tse_tspi_get_params,
