 ...
 *
 */
static int tse_pkcs11h_deserialize(struct pkcs11h_data *pkcs11h_data,
					unsigned char *blob)
{
 ...
 */
static int tse_pkcs11h_serialize(unsigned char *blob, size_t *blob_size,
				      struct pkcs11h_data *pkcs11h_data)
{
...
pkcs11h_pin_prompt (
	 ...
	 * Until we can affect tse context via daemon */
...
/**
 * tse_pkcs11h_get_public_key
 * @rsa: RSA key to allocate
 * @blob: Key module data to use in finding the key
 */
static int tse_pkcs11h_get_public_key(RSA **rsa, unsigned char *blob)
{
	...
	if ((rc = tse_pkcs11h_deserialize(pkcs11h_data, blob)) != 0) {
		goto out;
	}
...
static int tse_pkcs11h_get_key_sig(unsigned char *sig, unsigned char *blob)
{
	RSA *rsa = NULL;
	...
	if ((rc = tse_pkcs11h_get_public_key(&rsa, blob))) {
 ...
 * encryption.
 */
static int tse_pkcs11h_encrypt(char *to, size_t *to_size, char *from,
 ...
 */
static int tse_pkcs11h_decrypt(char *to, size_t *to_size, char *from,
				    size_t from_size, unsigned char *blob,
				    int blob_type)
{
	struct pkcs11h_data _pkcs11h_data;
...
static int tse_pkcs11h_process_key(struct pkcs11h_subgraph_key_ctx *subgraph_key_ctx,
			     struct val_node **mnt_params)
{
	...
	if ((rc = tse_pkcs11h_serialize(NULL, &blob_size, 
					     pkcs11h_data))) {
	...
	if ((rc = tse_pkcs11h_serialize(subgraph_key_ctx->key_mod->blob,
	...
	if ((rc = tse_add_key_module_key_to_keyring(sig, subgraph_key_ctx->key_mod)) < 0) {
		syslog(
			LOG_ERR,
...
static void
tf_tse_pkcs11h_destroy_subgraph_key_ctx(struct pkcs11h_subgraph_key_ctx *ctx)
{
...
static void
tf_tse_pkcs11h_destroy_subgraph_provider_ctx(struct pkcs11h_subgraph_provider_ctx *ctx)
{
	if (ctx->name != NULL) {
		free(ctx->name);
	}
...
static int tf_pkcs11h_global_loglevel(struct tse_ctx *ctx, struct param_node *node,
...
static int tf_pkcs11h_global_pincache(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	int rc;
...
static int tf_pkcs11h_provider(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
...
static int tf_pkcs11h_provider_name(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_provider_ctx *subgraph_provider_ctx;
	int rc;
...
static int tf_pkcs11h_provider_library(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_provider_ctx *subgraph_provider_ctx;
	int rc;
...
static int tf_pkcs11h_provider_prot_auth(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_provider_ctx *subgraph_provider_ctx;
	int rc;
...
static int tf_pkcs11h_provider_cert_private(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_provider_ctx *subgraph_provider_ctx;
	int rc;
...
static int tf_pkcs11h_provider_private_mask(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_provider_ctx *subgraph_provider_ctx;
	int rc;
...
static int tf_pkcs11h_provider_end(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_provider_ctx *subgraph_provider_ctx;
	CK_RV rv = CKR_FUNCTION_FAILED;
	...
	tf_tse_pkcs11h_destroy_subgraph_provider_ctx(subgraph_provider_ctx);
...
static int tf_pkcs11h_key_id(struct tse_ctx *ctx, struct param_node *node,
		       struct val_node **mnt_params, void **foo)
{
...
static int tf_pkcs11h_key_passwd(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_key_ctx *subgraph_key_ctx;
	int rc;
...
static int tf_pkcs11h_key_x509file(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct pkcs11h_subgraph_key_ctx *subgraph_key_ctx;
	X509 *x509 = NULL;
	...
	node->val = NULL;
	if ((rc = tse_pkcs11h_process_key(subgraph_key_ctx, mnt_params))) {
		syslog(LOG_ERR, "PKCS#11: Error processing PKCS#11 key; rc=[%d]", rc);
		goto out;
	}
	tf_tse_pkcs11h_destroy_subgraph_key_ctx(subgraph_key_ctx);
...
static struct param_node pkcs11h_provider_param_nodes[] = {
	...
	{.num_mnt_opt_names = 1,
	 ...
	 .display_opts = NULL,
	 .default_val = "1",
	 .flags = TSE_PARAM_FLAG_ECHO_INPUT  | TSE_ALLOW_IMPLICIT_TRANSITION,
	 .num_transitions = 1,
	 .tl = {{.val = "default",
		 .pretty_val = NULL,
 ...
 * subgraph.
 */
static int tf_pkcs11h_key_enter(struct tse_ctx *ctx,
			    ...
			    struct val_node **mnt_params, void **foo)
{
	...
	if ((rc = tse_find_key_mod(&subgraph_key_ctx->key_mod, ctx,
					param_node->val))) {
...
static int tse_pkcs11h_get_param_subgraph_trans_node(
...
static int tse_pkcs11h_parse_file(struct param_node *param_nodes)
{
	struct tse_ctx _ctx;
	struct tse_ctx *ctx = &_ctx;
	...
	if (asprintf(&rcfile_fullpath, "%s/.tserc.pkcs11", pw->pw_dir) == -1) {
		rc = -ENOMEM;
		goto out;
	}
	...
	if (tse_verbosity) {
		struct tse_name_val_pair *nvp_item = &nvp_head;
	...
	tse_eval_decision_graph(ctx, &dummy_mnt_params, param_nodes,
				     &nvp_head);
...
static int tse_pkcs11h_init(char **alias)
{
	CK_RV rv = CKR_FUNCTION_FAILED;
	int rc = 0;
	...
	tse_pkcs11h_parse_file(pkcs11h_global_param_nodes);
	...
	tse_pkcs11h_parse_file(pkcs11h_provider_param_nodes);
...
static int tse_pkcs11h_finalize(void)
{
	if (pkcs11h_key_param_nodes[PKCS11H_KEY_TOK_ID].suggested_val)
		free(pkcs11h_key_param_nodes[PKCS11H_KEY_TOK_ID].suggested_val);
	pkcs11h_terminate ();
...
static struct tse_key_mod_ops tse_pkcs11h_ops = {
	.init = &tse_pkcs11h_init,
	.get_gen_key_params = NULL,
	.get_gen_key_subgraph_trans_node = NULL,
	.get_params = NULL,
	.get_param_subgraph_trans_node = &tse_pkcs11h_get_param_subgraph_trans_node,
	.get_blob = NULL,
	.get_key_data = NULL,
	.get_key_sig = &tse_pkcs11h_get_key_sig,
	.get_key_hint = NULL,
	.encrypt = &tse_pkcs11h_encrypt,
	.decrypt = &tse_pkcs11h_decrypt,
	.destroy = NULL,
	.finalize = &tse_pkcs11h_finalize
};

struct tse_key_mod_ops *get_key_mod_ops(void)
{
	return &tse_pkcs11h_ops;
}
