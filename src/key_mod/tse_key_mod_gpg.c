...
static int
tse_gpg_initialize_key_module_state(unsigned char *blob,
					 struct tse_name_val_pair *pair)
...
static int
tse_gpg_get_key_metadata(char *sig, int *length, unsigned char *blob)
{
	struct key_mod_gpg key_mod_gpg;
	int rc = 0;
	...
	memcpy(sig, key_mod_gpg.sig, TSE_SIG_SIZE_HEX + 1);
	(*length) = 0; /* TODO */
out:
...
int tse_gpg_generate_key(char *filename)
{
	int rc = 0;
...
int tse_gpg_encrypt(char *to, int size, char *from, unsigned char *blob)
{
	int rc = 0;
...
int tse_gpg_decrypt(char *to, size_t *decrypted_key_size, char *from, 
			 unsigned char *blob)
{
/*	gpgme_key_t key;
	int rc;
...
static struct pki_nvp_map_elem pki_nvp_map[] = {
	{"gpgsig", (TSE_PARAM_FLAG_ECHO_INPUT
		    | TSE_DEFAULT_VALUE_SET)},
	{NULL, 0}
};

/*

static int ssl_sig(struct tse_pki_elem *pki, struct val_node **head)
{
	struct tse_name_val_pair *openssl_nvp;
	char *sig;
	char *param;
	int rc;

	sig = malloc(TSE_SIG_SIZE_HEX + 1);
	if (!sig) {
		rc = -ENOMEM;
		goto out;
	}
	rc = tse_add_key_module_key_to_keyring(sig, pki);
	if (rc < 0)
		goto out;
	else
		rc = 0;
	asprintf(&param, "tse_sig=%s", sig);
	free(sig);
...
static int tf_ssl_file(struct tse_ctx *ctx, struct param_node *node,
...
static int tf_gpg_keysig(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **head, void **foo);
...
static int generate_name_val_list(struct tse_name_val_pair *head)
{
	uid_t id = getuid();
	int rc = 0;
...
static int tf_gpg_exit(struct tse_ctx *ctx, struct param_node *node,
		       struct val_node **head, void **foo)
{
	struct key_mod_gpg *key_mod_gpg;
...
static int tf_gpg_enter(struct tse_ctx *ctx, struct param_node *node,
			    struct val_node **head, void **foo)
{
	struct key_mod_gpg *key_mod_gpg;
	gpgme_error_t err;
...
static int
tse_gpg_get_param_subgraph_trans_node(struct transition_node **trans,
					   uint32_t version)
{
...
static int tse_gpg_init(char **alias)
{
	uid_t id;
	struct passwd *pw;
	int rc = 0;
...
int tse_gpg_finalize(void)
{
	return 0;
}

static struct tse_key_mod_ops tse_gpg_ops = {
	&tse_gpg_init,
	NULL,
	NULL,
	NULL,
