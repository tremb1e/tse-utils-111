...
static void
tse_openssl_destroy_openssl_data(struct openssl_data *openssl_data)
{
 ...
 *
 */
static int tse_openssl_deserialize(struct openssl_data *openssl_data,
					unsigned char *blob)
{
	size_t path_length;
 ...
 */
static int tse_openssl_serialize(unsigned char *blob, size_t *blob_size,
...
struct tse_subgraph_ctx {
	struct tse_key_mod *key_mod;
	struct openssl_data openssl_data;
};

static void
tse_openssl_destroy_subgraph_ctx(struct tse_subgraph_ctx *ctx)
{
	tse_openssl_destroy_openssl_data(&ctx->openssl_data);
	memset(ctx, 0, sizeof(struct tse_subgraph_ctx));
}

/**
 * tse_openssl_generate_signature
 * @sig: Generated sig (TSE_SIG_SIZE_HEX + 1 bytes of allocated memory)
 * @key: RSA key from which to generate sig
 */
static int tse_openssl_generate_signature(char *sig, RSA *key)
{
	...
	to_hex(sig, (char *)hash, TSE_SIG_SIZE);
	sig[TSE_SIG_SIZE_HEX] = '\0';
out:
	free(data);
	free(hash);
	return rc;
...
static int
tse_openssl_mkdir_recursive(char *dir, mode_t mode)
{
	char *temp = NULL;
	char *parent = NULL;
	int rc = 0;
	...
	parent = dirname(temp);
	rc = tse_openssl_mkdir_recursive(parent, mode);
	if (rc)
		goto out;
	if (mkdir(dir, mode) == -1) {
		if (errno != EEXIST) {
...
static int
tse_openssl_write_key_to_file(RSA *rsa, char *filename, char *passphrase)
{
	char *tmp_filename;
	char *openssl_dir;
	BIO *out;
	...
	rc = tse_openssl_mkdir_recursive(openssl_dir, 0700);
	if (rc) {
...
/**
 * tse_openssl_read_key
 * @rsa: RSA key to allocate
 * @blob: Key module data to use in finding the key
 */
static int tse_openssl_read_key(RSA **rsa, unsigned char *blob)
{
	...
	tse_openssl_deserialize(openssl_data, blob);
...
int tse_openssl_get_key_sig(unsigned char *sig, unsigned char *blob)
{
	RSA *rsa = NULL;
	int rc;

	if ((rc = tse_openssl_read_key(&rsa, blob))) {
		syslog(LOG_ERR, "Error attempting to read RSA key from file;"
		       " rc = [%d]\n", rc);
		goto out;
	}
	if ((rc = tse_openssl_generate_signature((char *)sig, rsa))) {
...
static int tse_openssl_generate_key(struct openssl_data *openssl_data)
{
	...
	if ((rc = tse_openssl_write_key_to_file(
 ...
 * encryption.
 */
static int tse_openssl_encrypt(char *to, size_t *to_size, char *from,
 ...
 */
static int tse_openssl_decrypt(char *to, size_t *to_size, char *from,
				    size_t from_size, unsigned char *blob,
				    int blob_type)
{
	RSA *rsa = NULL;
...
int tse_openssl_init_from_param_vals(struct openssl_data *, struct key_mod_param_val *, uint32_t);
static int tse_openssl_get_blob(unsigned char *blob, size_t *blob_size,
				     ...
				     uint32_t num_param_vals)
{
	...
	if ((rc = tse_openssl_init_from_param_vals(&openssl_data,
							param_vals,
							num_param_vals))) {
	...
	if (blob == NULL) {
		if ((rc = tse_openssl_serialize(NULL, blob_size,
						     &openssl_data))) {
			syslog(LOG_ERR,
	...
	if ((rc = tse_openssl_serialize(blob, blob_size, &openssl_data))) {
...
static int tf_ssl_keyfile(struct tse_ctx *ctx, struct param_node *node,
			  struct val_node **mnt_params, void **foo)
{
	struct tse_subgraph_ctx *subgraph_ctx;
	int rc;

	subgraph_ctx = (struct tse_subgraph_ctx *)(*foo);
...
static int
tse_openssl_process_key(struct tse_subgraph_ctx *subgraph_ctx,
			     struct val_node **mnt_params)
{
	...
	if ((rc = tse_openssl_serialize((unsigned char *) 
	...
	if ((rc = tse_add_key_module_key_to_keyring(
	...
	if ((rc = asprintf(&sig_mnt_opt, "tse_sig=%s", sig)) == -1) {
		rc = -ENOMEM;
		goto out;
	}
			  ...
			  struct tse_subgraph_ctx *subgraph_ctx)
{
	...
	if ((rc=tse_openssl_read_key(&rsa, 
 ...
 * 
 */
static int tf_ssl_passwd(struct tse_ctx *ctx, struct param_node *node,
			 struct val_node **mnt_params, void **foo)
{
	struct tse_subgraph_ctx *subgraph_ctx;
	int rc;
	...
	free(node->val);
	node->val = NULL;
	if ((rc = tse_openssl_process_key(subgraph_ctx, mnt_params))) {
		syslog(LOG_ERR, "Error processing OpenSSL key; rc = [%d]", rc);
		goto out;
	}
	limit_key_size(mnt_params, subgraph_ctx);
	tse_openssl_destroy_subgraph_ctx(subgraph_ctx);
	free(subgraph_ctx);
	(*foo) = NULL;
	rc = DEFAULT_TOK;
out:
...
static int tf_ssl_passwd_file(struct tse_ctx *ctx, struct param_node *node,
			      struct val_node **mnt_params, void **foo)
{
	int rc = 0;
	int fd;
	struct tse_subgraph_ctx *subgraph_ctx;
	struct tse_name_val_pair file_head = { 0, };
	struct tse_name_val_pair *walker = NULL;
...
static int tf_ssl_passwd_fd(struct tse_ctx *ctx, struct param_node *node,
			    struct val_node **mnt_params, void **foo)
{
	return ENOSYS;
}

static int tf_tse_openssl_gen_key_param_node_keyfile(
	struct tse_ctx *ctx, struct param_node *node,
	struct val_node **mnt_params, void **foo)
{
	struct tse_subgraph_ctx *subgraph_ctx;	
	int rc = DEFAULT_TOK;
...
static int tf_tse_openssl_gen_key_param_node_passphrase(
	struct tse_ctx *ctx, struct param_node *node,
	struct val_node **mnt_params, void **foo)
{
	struct tse_subgraph_ctx *subgraph_ctx;	
	...
	if ((rc = tse_openssl_generate_key(&subgraph_ctx->openssl_data))) {
...
#define TSE_OPENSSL_GEN_KEY_PARAM_NODE_KEYFILE 0
#define TSE_OPENSSL_GEN_KEY_PARAM_NODE_PASSPHRASE 1
static struct param_node tse_openssl_gen_key_param_nodes[] = {
	{.num_mnt_opt_names = 1,
	 ...
	 .tl = {{.val = "default",
		 .pretty_val = "",
		 .next_token = &tse_openssl_gen_key_param_nodes[
			 TSE_OPENSSL_GEN_KEY_PARAM_NODE_PASSPHRASE],
		 .trans_func = &tf_tse_openssl_gen_key_param_node_keyfile}}},
	{.num_mnt_opt_names = 1,
	 ...
	 .default_val = NULL,
	 .suggested_val = NULL,
	 .flags = TSE_PARAM_FLAG_MASK_OUTPUT | VERIFY_VALUE,
	 .num_transitions = 1,
	 .tl = {{.val = NULL,
		 .pretty_val = NULL,
		 .next_token = NULL,
		 .trans_func =
		         &tf_tse_openssl_gen_key_param_node_passphrase}}}
};
...
static struct param_node ssl_param_nodes[] = {
	...
	{.num_mnt_opt_names = 2,
	 ...
	 .display_opts = NULL,
	 .default_val = NULL,
	 .flags = STDIN_REQUIRED | TSE_PARAM_FLAG_MASK_OUTPUT,
	 .num_transitions = 1,
	 .tl = {{.val = NULL,
		 .pretty_val = NULL,
		 .next_token = NULL,
	...
	{.num_mnt_opt_names = 2,
	 ...
	 .display_opts = NULL,
	 .default_val = NULL,
	 .flags = STDIN_REQUIRED | TSE_PARAM_FLAG_ECHO_INPUT
...
/**
 * tf_openssl_enter
 * @ctx: The current applicable libtse context struct
 ...
 * subgraph.
 */
static int tf_openssl_enter(struct tse_ctx *ctx,
			    ...
			    struct val_node **mnt_params, void **foo)
{
	...
	if ((subgraph_ctx = malloc(sizeof(struct tse_subgraph_ctx)))
	    == NULL) {
		rc = -ENOMEM;
		goto out;
	}
	memset(subgraph_ctx, 0, sizeof(struct tse_subgraph_ctx));
	if ((rc = tse_find_key_mod(&subgraph_ctx->key_mod, ctx,
					param_node->val))) {
...
static int tse_openssl_get_param_subgraph_trans_node(
	struct transition_node **trans, uint32_t version)
{
	if ((version & TSE_VERSIONING_PUBKEY) == 0)
		return -1;
...
struct transition_node openssl_gen_key_transition = {
	...
	.pretty_val = "OpenSSL module",
	.next_token = &(tse_openssl_gen_key_param_nodes[
				TSE_OPENSSL_GEN_KEY_PARAM_NODE_KEYFILE]),
	.trans_func = tf_openssl_enter
};

static int tse_openssl_get_gen_key_param_subgraph_trans_node(
	struct transition_node **trans, uint32_t version)
{
	if ((version & TSE_VERSIONING_PUBKEY) == 0)
		return -1;
...
int tse_openssl_finalize(void)
{
	return 0;
}

static int tse_openssl_init(char **alias)
{
	uid_t id;
	struct passwd *pw;
	struct param_node *gen_key_keyfile_param_node =
		&tse_openssl_gen_key_param_nodes[
			TSE_OPENSSL_GEN_KEY_PARAM_NODE_KEYFILE];
	int rc = 0;
...
static struct tse_key_mod_ops tse_openssl_ops = {
	&tse_openssl_init,
	NULL,
	&tse_openssl_get_gen_key_param_subgraph_trans_node,
	NULL,
	&tse_openssl_get_param_subgraph_trans_node,
	&tse_openssl_get_blob,
	NULL,
	&tse_openssl_get_key_sig,
	NULL,
	&tse_openssl_encrypt,
	&tse_openssl_decrypt,
	NULL,
	&tse_openssl_finalize
};

struct tse_key_mod_ops *get_key_mod_ops(void)
{
	return &tse_openssl_ops;
}
