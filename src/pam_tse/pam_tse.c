...
static int file_exists_dottse(const char *homedir, char *filename)
{
	char *file_path;
	int rc = 0;
	struct stat s;
	if (asprintf(&file_path, "%s/.tse/%s", homedir, filename) == -1)
		return -ENOMEM;
...
static int wrap_passphrase_if_necessary(const char *username, uid_t uid, char *wrapped_pw_filename, char *passphrase, char *salt)
{
	...
	rc = asprintf(&unwrapped_pw_filename, "/dev/shm/.tse-%s", username);
	if (rc == -1) {
		syslog(LOG_ERR, "pam_tse: Unable to allocate memory\n");
		return -ENOMEM;
	}
	...
	if (stat(unwrapped_pw_filename, &s) == 0 && (s.st_uid == uid) &&
		...
		setuid(uid);
		rc = tse_wrap_passphrase_file(wrapped_pw_filename, passphrase, salt, unwrapped_pw_filename);
		if (rc != 0) {
			syslog(LOG_ERR, "pam_tse: Error wrapping cleartext password; " "rc = [%d]\n", rc);
		}
		return rc;
	}
	return 0;
...
{
	char passphrase[TSE_MAX_PASSPHRASE_BYTES + 1];
	uint8_t version;
	int rc;

	memset(passphrase, 0, sizeof(passphrase));

	rc = __tse_detect_wrapped_passphrase_file_version(
							wrapped_pw_filename,
							&version);
	if (rc)
		return rc;
	...
	rc = tse_unwrap_passphrase(passphrase, wrapped_pw_filename,
					wrapping_passphrase, salt);
	if (rc)
		return rc;

	return tse_wrap_passphrase(wrapped_pw_filename,
				   ...
				   const char **argv)
{
	...
	if (rc == PAM_SUCCESS) {
		...
		syslog(LOG_ERR, "pam_tse: Error getting passwd info for user [%s]; rc = [%ld]\n", username, rc);
		goto out;
	}
	...
	if (!auth_tok_sig) {
		rc = -ENOMEM;
		syslog(LOG_ERR, "pam_tse: Out of memory\n");
		goto out;
	}
	...
	if ((child_pid = fork()) == 0) {
		...
		if (passphrase == NULL) {
			syslog(LOG_ERR, "pam_tse: NULL passphrase; aborting\n");
			rc = -EINVAL;
			goto out_child;
		}
		if ((rc = tse_validate_keyring())) {
			syslog(LOG_WARNING, "pam_tse: Cannot validate keyring integrity\n");
		}
		rc = 0;
		if ((argc == 1)
			...
			if (wrap_passphrase_if_necessary(username, uid, wrapped_pw_filename, passphrase, salt) == 0) {
				syslog(LOG_DEBUG, "pam_tse: Passphrase file wrapped");
			} else {
				goto out_child;
			}
			if (rewrap_passphrase_if_necessary(wrapped_pw_filename, passphrase, salt)) {
				/* Non fatal condition. Log a warning. */
				syslog(LOG_WARNING, "pam_tse: Unable to rewrap passphrase file\n");
			}
		...
		if (rc) {
			syslog(LOG_ERR, "pam_tse: Error adding passphrase key token to user session keyring; rc = [%ld]\n", rc);
			goto out_child;
		}
		if (fork() == 0) {
			if ((rc = tse_set_zombie_session_placeholder())) {
				syslog(LOG_ERR, "pam_tse: Error attempting to create and register zombie process; rc = [%ld]\n", rc);
			}
		}
out_child:
		free(auth_tok_sig);
	...
	if (tmp_pid == -1)
		syslog(LOG_WARNING, "pam_tse: waitpid() returned with error condition\n");
out:
...
static int private_dir(pam_handle_t *pamh, int mount)
{
	...
	if (
	    (asprintf(&autofile, "%s/.tse/%s", pwd->pw_dir, a) < 0)
	     || autofile == NULL) {
		syslog(LOG_ERR, "pam_tse: Error allocating memory for autofile name");
		return 1;
        }
        if (
	    (asprintf(&sigfile, "%s/.tse/%s.sig", pwd->pw_dir,
	     PRIVATE_DIR) < 0) || sigfile == NULL) {
		syslog(LOG_ERR, "pam_tse: Error allocating memory for sigfile name");
		return 1;
        }
	if (stat(sigfile, &s) != 0) {
	...
	if ((pid = fork()) < 0) {
		syslog(LOG_ERR, "pam_tse: Error setting up private mount");
		return 1;
	}
	if (pid == 0) {
		if (mount == 1) {
				...
				syslog(LOG_ERR, "pam_tse: Error allocating memory for recorded name");
				exit(1);
			}
			...
			if (stat(autofile, &s) != 0) {
				...
				syslog(LOG_DEBUG, "pam_tse: Skipping automatic Tse mount");
				exit(0);
			}
			clearenv();
			...
			if (stat(autofile, &s) != 0) {
				...
				syslog(LOG_DEBUG, "pam_tse: Skipping automatic Tse unmount");
				exit(0);
			}
			clearenv();
			if (setgroups(1, &pwd->pw_gid) < 0 || setgid(pwd->pw_gid) < 0)
                                ...
                                int argc, const char **argv)
{
	...
	if (rc == PAM_SUCCESS) {
		...
		syslog(LOG_ERR, "pam_tse: Error getting passwd info for user [%s]; rc = [%d]\n", username, rc);
		goto out;
	}
	...
	if ((rc = pam_get_item(pamh, PAM_OLDAUTHTOK,
	    ...
	    != PAM_SUCCESS) {
		syslog(LOG_ERR, "pam_tse: Error retrieving old passphrase; rc = [%d]\n", rc);
		goto out;
	}
	...
	if ((flags & PAM_PRELIM_CHECK)) {
		if (!old_passphrase)
		{
			syslog(LOG_WARNING, "pam_tse: PAM passphrase change module retrieved a NULL passphrase; nothing to do\n");
	...
	if ((rc = pam_get_item(pamh, PAM_AUTHTOK,
	    ...
	    != PAM_SUCCESS) {
		syslog(LOG_ERR, "pam_tse: Error retrieving new passphrase; rc = [%d]\n", rc);
		goto out;
	}
	...
	if (!old_passphrase || !new_passphrase || *new_passphrase == '\0') {
		syslog(LOG_WARNING, "pam_tse: PAM passphrase change module retrieved at least one NULL passphrase; nothing to do\n");
		rc = PAM_AUTHTOK_RECOVER_ERR;
		goto out;
	}
	rc = PAM_SUCCESS;
	if ((child_pid = fork()) == 0) {
		...
		if ((rc = tse_unwrap_passphrase(passphrase,
						     wrapped_pw_filename,
						     old_passphrase, salt))) {
			syslog(LOG_ERR, "pam_tse: Error attempting to unwrap passphrase; rc = [%d]\n", rc);
			goto out_child;
		}
		if ((rc = tse_wrap_passphrase(wrapped_pw_filename,
						   new_passphrase, salt,
						   passphrase))) {
			syslog(LOG_ERR, "pam_tse: Error attempting to wrap passphrase; rc = [%d]", rc);
			goto out_child;
		}
out_child:
		exit(0);
