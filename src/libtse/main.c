...
void tse_get_versions(int *major, int *minor, int *file_version)
{
	*major = TSE_VERSION_MAJOR;
	*minor = TSE_VERSION_MINOR;
	if (file_version)
		*file_version = TSE_SUPPORTED_FILE_VERSION;
}
...
/* Read tse private mount from file
...
char *tse_fetch_private_mnt(char *pw_dir) {
	...
	if (asprintf(&mnt_default, "%s/%s", pw_dir, TSE_PRIVATE_DIR) < 0
			|| mnt_default == NULL) {
		perror("asprintf");
		return NULL;
	}
	if (
			asprintf(&mnt_file, "%s/.tse/%s.mnt", pw_dir, TSE_PRIVATE_DIR) < 0
			|| mnt_file == NULL) {
		perror("asprintf");
		return NULL;
	}
...
/* Check if an tse private device or mount point is mounted.
...
int tse_private_is_mounted(char *dev, char *mnt, char *sig, int mounting) {
	...
	char *opt = NULL;
	int mounted;
	if (sig && asprintf(&opt, "tse_sig=%s", sig) < 0) {
		perror("asprintf");
		return 0;
	}
	...
	while ((m = getmntent(fh)) != NULL) {
		if (strcmp(m->mnt_type, "tse") != 0)
			/* Skip if this entry is not an tse mount */
			continue;
		if (mounting == 1) {
 ...
 * @fekek: TSE_MAX_KEY_BYTES bytes of allocated memory
 *
...
{
	char salt_and_passphrase[TSE_MAX_PASSPHRASE_BYTES
				 + TSE_SALT_SIZE];
	int passphrase_size;
	int alg = SEC_OID_SHA512;
	int dig_len = SHA512_DIGEST_LENGTH;
	char buf[SHA512_DIGEST_LENGTH];
	int hash_iterations = TSE_DEFAULT_NUM_HASH_ITERATIONS;
	int rc = 0;

	passphrase_size = strlen(passphrase);
	if (passphrase_size < 1 ||
	    passphrase_size > TSE_MAX_PASSPHRASE_BYTES) {
		passphrase_sig = NULL;
		syslog(LOG_ERR, "Passphrase size is invalid; [1] to [%d] is the valid range\n",
		       TSE_MAX_PASSPHRASE_BYTES);
		return -EINVAL;
	}
	memcpy(salt_and_passphrase, salt, TSE_SALT_SIZE);
	memcpy((salt_and_passphrase + TSE_SALT_SIZE), passphrase,
		passphrase_size);
	if ((rc = do_hash(salt_and_passphrase,
			  (TSE_SALT_SIZE + passphrase_size), buf, alg))) {
		return rc;
	}
	hash_iterations--;
	...
	memcpy(fekek, buf, TSE_MAX_KEY_BYTES);
	if ((rc = do_hash(buf, dig_len, buf, alg))) {
		return rc;
	}
	to_hex(passphrase_sig, buf, TSE_SIG_SIZE);
	return 0;
}
		 ...
		 char *salt, char *session_key_encryption_key)
{
	...
	tse_get_versions(&major, &minor, NULL);
	auth_tok->version = (((uint16_t)(major << 8) & 0xFF00)
			     | ((uint16_t)minor & 0x00FF));
	auth_tok->token_type = TSE_PASSWORD;
	strncpy((char *)auth_tok->token.password.signature, passphrase_sig,
		TSE_PASSWORD_SIG_SIZE);
	memcpy(auth_tok->token.password.salt, salt, TSE_SALT_SIZE);
	memcpy(auth_tok->token.password.session_key_encryption_key,
	       session_key_encryption_key, TSE_MAX_KEY_BYTES);
	/* TODO: Make the hash parameterizable via policy */
	auth_tok->token.password.session_key_encryption_key_bytes =
		TSE_MAX_KEY_BYTES;
	auth_tok->token.password.flags |=
		TSE_SESSION_KEY_ENCRYPTION_KEY_SET;
	/* The kernel code will encrypt the session key. */
	auth_tok->session_key.encrypted_key[0] = 0;
	auth_tok->session_key.encrypted_key_size = 0;
	/* Default; subject to change by kernel Tse */
	auth_tok->token.password.hash_algo = PGP_DIGEST_ALGO_SHA512;
	auth_tok->token.password.flags &= ~(TSE_PERSISTENT_PASSWORD);
	return rc;
}
			      ...
			      char *sig, size_t blob_size)
{
	...
	auth_tok->version = (((uint16_t)(major << 8) & 0xFF00)
			     | ((uint16_t)minor & 0x00FF));
	auth_tok->token_type = TSE_PRIVATE_KEY;
	...
	if (key_data_len == 0) {
		...
		if ((rc = tse_generate_sig_from_key_data(
	...
	strncpy(auth_tok->token.private_key.key_mod_alias, key_mod->alias,
		TSE_MAX_KEY_MOD_NAME_BYTES);
	/* TODO: Get rid of this */
	auth_tok->token.private_key.key_size = TSE_MAX_KEY_MOD_NAME_BYTES;
	auth_tok->token.private_key.data_len = blob_size;
	memcpy(auth_tok->token.private_key.signature, sig,
	       TSE_SIG_SIZE_HEX);
	auth_tok->token.private_key.signature[TSE_SIG_SIZE_HEX] = '\0';
out:
	return rc;
}
...
static int zombie_semaphore_get(void)
{
	...
	sem_id = semget(TSE_SEM_KEY, 1, (0666 | IPC_EXCL | IPC_CREAT));
	if (sem_id >= 0) {
		sb.sem_op = 1;
		sb.sem_flg = 0;
		sb.sem_num = 0;
...
static int get_zombie_shared_mem_locked(int *shm_id, int *sem_id)
{
	...
	rc = shmget(TSE_SHM_KEY, TSE_SHM_SIZE, (0666 | IPC_CREAT
							  | IPC_EXCL));
	if (rc == -1 && errno == EEXIST)
		rc = shmget(TSE_SHM_KEY, TSE_SHM_SIZE, 0);
	else {
		char *shm_virt;
		...
		memset(shm_virt, 0, TSE_SHM_SIZE);
...
static int list_pid_sid_pairs(int shm_id)
{
	...
	while (!(sid_tmp == 0 && pid_tmp == 0)) {
		if ((i + (2 * sizeof(pid_t))) > TSE_SHM_SIZE)
			break;
		memcpy(&sid_tmp, &shm_virt[i], sizeof(pid_t));
		i += sizeof(pid_t);
		sid_tmp = ntohl(sid_tmp); /* uint32_t */
...
int tse_set_zombie_session_placeholder(void)
{
	...
	zombie_semaphore_unlock(sem_id);
	sleep(TSE_ZOMBIE_SLEEP_SECONDS);
	if ((rc = get_zombie_shared_mem_locked(&shm_id, &sem_id))) {
		syslog(LOG_ERR,
		       "Error getting shared memory segment\n");
		goto out;
...
static struct tse_ctx_ops ctx_ops;
