...
void tse_destroy_nvp(struct tse_name_val_pair *nvp)
{
	return;
}

int tse_delete_nvp(struct tse_name_val_pair *nvp_head,
...
/**
 * do_transition
 * @ctx: The current Tse library context
 ...
 *            .tserc file. Whenever a param node needs a value,
 ...
 */
int do_transition(struct tse_ctx *ctx, struct param_node **next,
		  ...
		  struct val_node **mnt_params, void **foo)
{
	...
	for (i = 0; i < current->num_transitions; i++) {
		...
		struct tse_name_val_pair *nvp = nvp_head->next;
			...
			struct param_node *node)
{
	...
	(*value_retrieved) = 0;
	while (i > 0) {
		struct tse_name_val_pair *temp = nvp_head->next;

		i--;
		while (temp) {
			if (strcmp(temp->name, node->mnt_opt_names[i]) == 0
			    && !(temp->flags & TSE_PROCESSED)) {
				if (tse_verbosity)
					syslog(LOG_INFO, "From param_node = "
					       "[%p]; mnt_opt_names[0] = [%s]"
					       ": Setting "
					       "TSE_PROCESSED to nvp with "
					       "nvp->name = [%s]\n",
 ...
 * being non-NULL.
 */
static int alloc_and_get_val(struct tse_ctx *ctx, struct param_node *node,
			     struct tse_name_val_pair *nvp_head)
{
	char *verify_prompt;
	char *verify;
	...
	if (value_retrieved) {
		if (tse_verbosity)
			syslog(LOG_INFO,
			       ...
			       __FUNCTION__);
		if (!(node->flags & TSE_ALLOW_IMPLICIT_TRANSITION
		      && node->flags & TSE_IMPLICIT_OVERRIDE_DEFAULT))
			goto out;
	}
	if (node->flags & TSE_ALLOW_IMPLICIT_TRANSITION
	    && !(node->flags & TSE_NO_AUTO_TRANSITION)) {
	...
	if (node->flags & TSE_PARAM_FLAG_NO_VALUE) {
		if (tse_verbosity)
			syslog(LOG_INFO,
			       "%s: TSE_PARAM_FLAG_NO_VALUE set\n",
			       __FUNCTION__);
		goto out;
	}
	...
	if (ctx->get_string) {
		...
		if (node->flags & DISPLAY_TRANSITION_NODE_VALS) {
			...
			for (i = 0; i < node->num_transitions; i++) {
				...
				pe = pe->next;
				memset(pe, 0, sizeof(*pe));
				if (node->flags & TSE_DISPLAY_PRETTY_VALS)
...
obtain_value:
			...
			if (rc)
				goto out;
			if (node->val[0] == '\0' && 
			    (node->flags & TSE_NONEMPTY_VALUE_REQUIRED)) {
...
static void get_verbosity(struct tse_name_val_pair *nvp_head,
			  int *verbosity)
{
	struct tse_name_val_pair *temp = nvp_head->next;
				   ...
				   int depth, int recursive)
{
	...
	if (recursive && trans_node->next_token)
		tse_dump_param_node(file_stream, trans_node->next_token,
					 depth + 1, recursive);
	print_whitespace(file_stream, depth);
	fprintf(file_stream, "---------------\n");
}
			      ...
			      int recursive)
{
	...
	for (i = 0; i < param_node->num_transitions; i++) {
		...
		tse_dump_transition_node(file_stream, &param_node->tl[i],
					      depth + 1, recursive);
	}
	print_whitespace(file_stream, depth);
	fprintf(file_stream, "----------\n");
...
void tse_dump_decision_graph(FILE *file_stream,
				  struct param_node *param_node, int depth)
{
	tse_dump_param_node(file_stream, param_node, depth, 1);
}

int tse_insert_params(struct tse_name_val_pair *nvp,
			   struct param_node *param_node)
{
	int i;
	struct tse_name_val_pair *cursor = nvp;
	int rc = 0;
	...
	for (i = 0; i < param_node->num_mnt_opt_names; i++) {
		if ((cursor->next =
		     malloc(sizeof(struct tse_name_val_pair))) == NULL) {
				       ...
				       struct transition_node *trans_node)
{
	...
	if (trans_node->next_token)
		rc = tse_insert_params(nvp, trans_node->next_token);
...
static struct flag_map {
	...
	{.flag_src = TSE_PARAM_FLAG_ECHO_INPUT,
	 .flag_dst = TSE_PARAM_FLAG_ECHO_INPUT},
};
#define TSE_NVP_FLAGS_TO_PARAM_FLAGS_MAP_SIZE 1

static int tse_map_flags(uint32_t *param_flags, uint32_t nvp_flags)
{
	int i;

	for (i = 0; i < TSE_NVP_FLAGS_TO_PARAM_FLAGS_MAP_SIZE; i++)
 ...
 */
static int
tse_linear_subgraph_val_tf(struct tse_ctx *ctx,
				  struct param_node *param_node,
				  struct val_node **mnt_params, void **foo)
{
 ...
 */
static int
tse_exit_linear_subgraph_tf(struct tse_ctx *ctx,
				 struct param_node *param_node,
				 struct val_node **mnt_params, void **foo)
{
 ...
 * tse_add_key_module_key_to_keyring() with this parameter value
 * list. This, in turn, calls tse_generate_key_payload(), which
 * calls the module's get_blob() function and takes steps to generate
 * the key signature. The exit callback appends an tse_sig=
 ...
 * libtse to whatever it wants to set it to.
 */
				   ...
				   struct tse_key_mod *key_mod)
{
	...
	tmp_tn->trans_func = &tse_enter_linear_subgraph_tf;
	rc = 0;
	param_node = NULL;
	for (i = 0; params && i < num_params; i++) {
		...
		tse_map_flags(&param_node->flags, params[i].flags);
		...
		tmp_tn->trans_func = &tse_linear_subgraph_val_tf;
	...
	param_node->num_mnt_opt_names = 1;
	param_node->flags |= TSE_PARAM_FLAG_NO_VALUE;
	tmp_tn->next_token = param_node;
	tmp_tn = &param_node->tl[0];
	if ((rc = asprintf(&tmp_tn->val, "default")) == -1) {
		rc = -ENOMEM;
	...
	rc = 0;
	param_node->num_transitions = 1;
	tmp_tn->trans_func = &tse_exit_linear_subgraph_tf;
out:
	return rc;
}
