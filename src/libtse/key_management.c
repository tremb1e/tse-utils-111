 ...
 * @auth_tok_sig: (out) Allocated memory this function fills in:
                        (TSE_SIG_SIZE_HEX + 1)
 * @fekek: (out) Allocated memory this function fills in: TSE_MAX_KEY_BYTES
 * @salt: (in) salt: TSE_SALT_SIZE
 * @passphrase: (in) passphrase: TSE_MAX_PASSPHRASE_BYTES
 */
int tse_generate_passphrase_auth_tok(struct tse_auth_tok **auth_tok,
					  ...
					  char *salt, char *passphrase)
{
	...
	*auth_tok = malloc(sizeof(struct tse_auth_tok));
	if (!*auth_tok) {
	...
	if (rc) {
		syslog(LOG_ERR, "Error generating payload for auth tok key; "
		       "rc = [%d]\n", rc);
		memset(*auth_tok, 0, sizeof(struct tse_auth_tok));
		free(*auth_tok);
		*auth_tok = NULL;
		rc = (rc < 0) ? rc : rc * -1;
		goto out;
...
/**
 * tse_passphrase_sig_from_blob
 * @blob: Byte array of struct tse_auth_tok
 *
...
binary_data tse_passphrase_sig_from_blob(char *blob)
{
	...
	auth_tok = (struct tse_auth_tok *)blob;
	bd.size = (TSE_PASSWORD_SIG_SIZE + 1);
...
binary_data tse_passphrase_blob(char *salt, char *passphrase)
{
	...
	char fekek[TSE_MAX_KEY_BYTES];
	binary_data bd;
	int rc;

	memset(&bd, 0, sizeof(bd));
	rc = tse_generate_passphrase_auth_tok(&auth_tok, auth_tok_sig,
	...
	bd.size = sizeof(struct tse_auth_tok);
	bd.data = blob;
out:
	return bd;
}
				     ...
				     char *auth_tok_sig)
{
	...
	rc = add_key("user", auth_tok_sig, (void *)auth_tok,
		     sizeof(struct tse_auth_tok), KEY_SPEC_USER_KEYRING);
	if (rc == -1) {
		rc = -errno;
...
/**
 * tse_add_blob_to_keyring
 * @blob: Byte array containing struct tse_auth_tok
...
int tse_add_blob_to_keyring(char *blob, char *sig)
{
	...
	rc = tse_add_auth_tok_to_keyring((struct tse_auth_tok *)blob,
					      sig);
	return rc;
}
					   ...
					   char *salt)
{
	int rc;
	char fekek[TSE_MAX_KEY_BYTES];
	struct tse_auth_tok *auth_tok = NULL;
	...
	rc = tse_add_auth_tok_to_keyring(auth_tok, auth_tok_sig);
	if (rc < 0) {
...
/**
 * __tse_detect_wrapped_passphrase_file_version
 * @filename: The path of a wrapped passphrase file
 * @version: On success, *version is set to the detected file version
 *
 * THIS FUNCTION IS NOT PART OF THE LIBTSE PUBLIC API. Code external to
 * tse-utils should not use it.
 *
				  ...
				  char *salt, char *src)
{
	...
	close(fd);
	if (tse_wrap_passphrase(dest, wrapping_passphrase, salt,
...
static int write_v2_wrapped_passphrase_file(const char *filename,
			const char wrapping_salt[TSE_SALT_SIZE],
			const char wrapping_key_sig[TSE_SIG_SIZE_HEX],
			...
			int encrypted_passphrase_bytes)
{
	...
	size = do_write(fd, wrapping_salt, TSE_SALT_SIZE);
	if (size != TSE_SALT_SIZE) {
		rc = size == -1 ? -errno : -EIO;
		goto out;
	}

	size = do_write(fd, wrapping_key_sig, TSE_SIG_SIZE_HEX);
	if (size != TSE_SIG_SIZE_HEX) {
		rc = size == -1 ? -errno : -EIO;
		goto out;
	}
			     ...
			     char *unused, char *decrypted_passphrase)
{
	...
	char wrapping_auth_tok_sig[TSE_SIG_SIZE_HEX + 1];
	char wrapping_key[TSE_MAX_KEY_BYTES];
	char padded_decrypted_passphrase[TSE_MAX_PASSPHRASE_BYTES +
		TSE_AES_BLOCK_SIZE + 1];
	char encrypted_passphrase[TSE_MAX_PASSPHRASE_BYTES +
		TSE_AES_BLOCK_SIZE + 1];
	...
	if (decrypted_passphrase_bytes < 1 ||
	    decrypted_passphrase_bytes > TSE_MAX_PASSPHRASE_BYTES) {
		syslog(LOG_ERR, "Decrypted passphrase size is invalid; [1] to [%d] is the valid range\n",
		       TSE_MAX_PASSPHRASE_BYTES);
		rc = -EIO;
		goto out;
	}
	rc = read_urandom(wrapping_salt, TSE_SALT_SIZE);
	if (rc != TSE_SALT_SIZE) {
		rc = rc == -1 ? -errno : -EIO;
	...
	memset(padded_decrypted_passphrase, 0,
	       (TSE_MAX_PASSPHRASE_BYTES + 1));
	memcpy(padded_decrypted_passphrase, decrypted_passphrase,
	       decrypted_passphrase_bytes);
	if ((decrypted_passphrase_bytes % TSE_AES_BLOCK_SIZE) != 0)
		decrypted_passphrase_bytes += (TSE_AES_BLOCK_SIZE
					       - (decrypted_passphrase_bytes
						  % TSE_AES_BLOCK_SIZE));
	...
	key_item.len = TSE_AES_KEY_BYTES;
	...
	err = PK11_CipherOp(
		...
		&tmp1_outlen, TSE_MAX_PASSPHRASE_BYTES +
		  TSE_AES_BLOCK_SIZE,
	...
	err = PK11_DigestFinal(
		...
		(TSE_MAX_PASSPHRASE_BYTES + 
		  TSE_AES_BLOCK_SIZE - tmp1_outlen));
	if (err == SECFailure) {
...
static int read_v1_wrapped_passphrase_file(const char *filename,
				char wrapping_key_sig[TSE_SIG_SIZE_HEX],
				char encrypted_passphrase[TSE_MAX_PASSPHRASE_BYTES],
				int *encrypted_passphrase_bytes)
{
	ssize_t size;
	...
	*encrypted_passphrase_bytes = 0;
	...
	if ((size = do_read(fd, wrapping_key_sig,
			    TSE_SIG_SIZE_HEX)) < TSE_SIG_SIZE_HEX) {
		syslog(LOG_ERR,
				...
				int *encrypted_passphrase_bytes)
{
	...
	*encrypted_passphrase_bytes = 0;
	...
	size = do_read(fd, wrapping_salt, TSE_SALT_SIZE);
	if (size != TSE_SALT_SIZE) {
	...
	size = do_read(fd, wrapping_key_sig, TSE_SIG_SIZE_HEX);
	if (size != TSE_SIG_SIZE_HEX) {
		rc = size == -1 ? errno : -EINVAL;
		goto out;
	}
	...
	size = do_read(fd, encrypted_passphrase, TSE_MAX_PASSPHRASE_BYTES);
	if (size < 0) {
		rc = size;
		goto out;
	} else if(size == 0) {
...
/**
 * dtse_passphrase must be able to hold
 * TSE_MAX_PASSPHRASE_BYTES + 1 bytes
 */
int tse_unwrap_passphrase(char *decrypted_passphrase, char *filename,
			       char *wrapping_passphrase, char *wrapping_salt)
{
	char v2_wrapping_salt[TSE_SALT_SIZE];
	char wrapping_auth_tok_sig[TSE_SIG_SIZE_HEX + 1];
	char wrapping_auth_tok_sig_from_file[TSE_SIG_SIZE_HEX + 1];
	char wrapping_key[TSE_MAX_KEY_BYTES];
	char encrypted_passphrase[TSE_MAX_PASSPHRASE_BYTES + 1];
	int encrypted_passphrase_pos = 0;
	int decrypted_passphrase_pos = 0;
	int tmp1_outlen = 0;
	int tmp2_outlen = 0;
	...
	enc_ctx = PK11_CreateContextBySymKey(CKM_AES_ECB, CKA_DECRYPT,
					     sym_key, sec_param);
	memset(decrypted_passphrase, 0, TSE_MAX_PASSPHRASE_BYTES + 1);
	err = PK11_CipherOp(
		enc_ctx, (unsigned char *) decrypted_passphrase,
		&tmp1_outlen, TSE_MAX_PASSPHRASE_BYTES,
		(unsigned char *) encrypted_passphrase,
		encrypted_passphrase_bytes);//TSE_MAX_PASSPHRASE_BYTES);
	if (err == SECFailure) {
		syslog(LOG_ERR, "%s: PK11_CipherOp() error; "
			"SECFailure = [%d]; PORT_GetError() = [%d]\n",
			__FUNCTION__, SECFailure, PORT_GetError());
	...
	err = PK11_DigestFinal(
		...
		(unsigned int *) &tmp2_outlen,
		(TSE_MAX_PASSPHRASE_BYTES - tmp1_outlen));
	if (err == SECFailure) {
		syslog(LOG_ERR, "%s: PK11 error on digest final; "
			"SECFailure = [%d]; PORT_GetError() = [%d]\n",
			__FUNCTION__, SECFailure, PORT_GetError());
...
/**
 * tse_insert_wrapped_passphrase_into_keyring()
 *
	...
	char *salt)
{
	...
	if ((rc = tse_unwrap_passphrase(decrypted_passphrase, filename,
	...
	if ((rc = tse_add_passphrase_key_to_keyring(auth_tok_sig,
...
/**
 * tse_add_key_module_key_to_keyring
 * @auth_tok_sig: (TSE_SIG_SIZE_HEX + 1) bytes of allocated
 ...
 */
int
tse_add_key_module_key_to_keyring(char *auth_tok_sig,
				       struct tse_key_mod *key_mod)
{
	...
	if ((auth_tok = malloc(sizeof(struct tse_auth_tok) + blob_size))
	    == NULL) {
		rc = -ENOMEM;
		goto out;
	}
	if ((rc = tse_generate_key_payload(auth_tok, key_mod, auth_tok_sig,
						blob_size))) {
	...
	rc = add_key("user", auth_tok_sig, (void *)auth_tok,
		     (sizeof(struct tse_auth_tok) + blob_size),
...
out:
	if (auth_tok != NULL) {
		memset(auth_tok, 0, (sizeof(struct tse_auth_tok) + blob_size));
		free(auth_tok);
	}
	return rc;
}
...
{
	struct tse_name_val_pair nvp_list_head;
	struct tse_name_val_pair *nvp;
	int rc;

	memset(&nvp_list_head, 0, sizeof(struct tse_name_val_pair));
	rc = tse_parse_rc_file(&nvp_list_head);
	if (rc) {
		if (rc != -ENOENT && rc != -EACCES) {
			syslog(LOG_WARNING,
				"Error attempting to parse .tserc file; "
				"rc = [%d]", rc);
		}
		goto out;
	}
	...
	while (nvp) {
		if (strcmp(nvp->name, "salt") == 0) {
			...
			if (valsize != TSE_SALT_SIZE_HEX)
				goto next_iteration;
			memcpy(salt_hex, nvp->value, TSE_SALT_SIZE_HEX);
			goto out_free;
		}
next_iteration:
		nvp = nvp->next;
...
{
	int fd;
	char tmp[TSE_SIG_SIZE_HEX + 1];
	ssize_t size;
	int rc = 0;

	memset(tmp, 0, sizeof(tmp));
	(*flags) &= ~TSE_SIG_FLAG_NOENT;
	fd = open(sig_cache_filename, O_RDONLY);
	if (fd == -1) {
		(*flags) |= TSE_SIG_FLAG_NOENT;
		goto out;
	}
	while ((size = read(fd, tmp, (TSE_SIG_SIZE_HEX + 1)))
	       == (TSE_SIG_SIZE_HEX + 1)) {
		if (memcmp(auth_tok_sig, tmp, TSE_SIG_SIZE_HEX)
		    == 0) {
			close(fd);
			goto out;
		}
...
int tse_append_sig(char *auth_tok_sig, char *sig_cache_filename)
{
	...
	lseek(fd, 0, SEEK_END);
	memcpy(tmp, auth_tok_sig, TSE_SIG_SIZE_HEX);
	tmp[TSE_SIG_SIZE_HEX] = '\n';
	if ((size = write(fd, tmp, (TSE_SIG_SIZE_HEX + 1))) !=
	    (TSE_SIG_SIZE_HEX + 1)) {
...
char *tse_get_passphrase(char *prompt) {
	...
	if ((passphrase =
	    (char *)malloc(TSE_MAX_PASSWORD_LENGTH+2)) == NULL) {
		perror("malloc");
		printf("\n");
		return NULL;
	}
	...
	tse_disable_echo(&current_settings);
	if (fgets(passphrase,
		  TSE_MAX_PASSWORD_LENGTH+2, stdin) == NULL) {
		tse_enable_echo(&current_settings);
		printf("\n");
		free(passphrase);
		return NULL;
	}
...
char *tse_get_wrapped_passphrase_filename() {
	...
	if ((asprintf(&filename,
	    "%s/.tse/wrapped-passphrase", pwd->pw_dir) < 0)) {
		perror("asprintf");
		return NULL;
	}
