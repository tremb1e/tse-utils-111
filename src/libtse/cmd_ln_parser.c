...
int print_nvp_list(struct tse_name_val_pair *dst)
{
...
static int copy_nv_pair(struct tse_name_val_pair *dst,
...
{
	int rc = 0;
	struct tse_name_val_pair *dst_cursor;
	struct tse_name_val_pair *src_cursor;

	src_cursor = src->next;
	while (src_cursor) {
		int found_match;
		struct tse_name_val_pair *prev_dst_cursor;
...
do_insert:
		if (!found_match) {
			struct tse_name_val_pair *dst_tmp;
			struct tse_name_val_pair *src_tmp;
			int i;

			prev_dst_cursor->next = dst_cursor =
				malloc(sizeof(struct tse_name_val_pair));
			memset(dst_cursor, 0,
			       sizeof(struct tse_name_val_pair));
			if (!dst_cursor) {
				rc = -ENOMEM;
				goto out;
			}
...
int
tse_parse_rc_file_fullpath(struct tse_name_val_pair *nvp_list_head,
				char *fullpath)
{
	int rc;
	int fd;
...
int tse_parse_rc_file(struct tse_name_val_pair *nvp_list_head)
{
	...
	home = pw->pw_dir;
	rc = asprintf(&rcfile_fullpath, "%s/.tserc", home);
	if (rc == -1) {
		rc = -ENOMEM;
		goto out;
	}
	rc = tse_parse_rc_file_fullpath(nvp_list_head, rcfile_fullpath);
	free(rcfile_fullpath);
out:
	return rc;
}

int process_comma_tok(struct tse_name_val_pair **current, char *tok,
		      /*@null@*/ char *prefix)
{
	int tok_len;
...
process_nv_pair:
	...
	memset((*current)->next, 0, sizeof(struct tse_name_val_pair));
	if (strlen(name) == 0) {
		free(name);
		free(value);
	} else {
 ...
 */
int generate_nv_list(struct tse_name_val_pair *head, char *buf)
{
	struct tse_name_val_pair *current = head;
