...
#define TSE_PACKET_STATUS_GOOD 0
#define TSE_PACKET_STATUS_BAD -1
...
key_mod_encrypt(char **encrypted_key, size_t *encrypted_key_size,
		struct tse_ctx *ctx, struct tse_auth_tok *auth_tok,
		char *decrypted_key, size_t decrypted_key_size)
{
	struct tse_key_mod *key_mod;
	int rc;

	if (tse_find_key_mod(&key_mod, ctx,
	 ...
	 * worry about TSE_MAX_ENCRYPTED_KEY_BYTES until the
	 * second call. */
	...
	if ((*encrypted_key_size) > TSE_MAX_ENCRYPTED_KEY_BYTES) {
		rc = -EINVAL;
		syslog(LOG_ERR, "Encrypted key size reported by key module "
		       "encrypt function is [%zu]; max is [%d]\n",
		       (*encrypted_key_size), TSE_MAX_ENCRYPTED_KEY_BYTES);
		free(*encrypted_key);
		...
		char *encrypted_key, size_t encrypted_key_size)
{
	...
	/* The first call just told us how much memory to
	 * allocate. The actual key size may be less, so we don't
	 * worry about TSE_MAX_KEY_BYTES until the second
	 * call. */
	...
	if ((*decrypted_key_size) > TSE_MAX_KEY_BYTES) {
		rc = -EINVAL;
		syslog(LOG_ERR, "Decrypted key size reported by key module "
		       "decrypt function is [%zu]; max is [%d]\n",
		       (*decrypted_key_size), TSE_MAX_KEY_BYTES);
		free(*decrypted_key);
				...
				struct tse_message **reply)
{
	...
	*reply = malloc(sizeof(struct tse_message) + 2);
	if (!*reply) {
		rc = -errno;
		syslog(LOG_ERR, "Failed to allocate memory: %m\n");
		goto out;
			       ...
			       struct tse_message **reply)
{
	...
	data_len = key_size + 4;
	*reply = malloc(sizeof(struct tse_message) + data_len);
	if (!*reply) {
		rc = -errno;
		syslog(LOG_ERR, "Failed to allocate memory: %m\n");
		goto out;
	...
	data = (*reply)->data;
	data[i++] = TSE_TAG_65_PACKET;
	data[i++] = TSE_PACKET_STATUS_GOOD;
	rc = tse_write_packet_length((char*)&data[i], key_size, &length_size);
	if (rc) {
		 ...
		 struct tse_message **reply)
{
	...
	if ((rc = tse_parse_packet_length(&emsg->data[i], &data_size,
					       &length_size))) {
		syslog(LOG_ERR, "Invalid packet format\n");
		goto write_failure;
	}
	...
	i += data_size;
	rc = tse_parse_packet_length(&emsg->data[i], &key_size,
					  &length_size);
	if (rc) {
		syslog(LOG_ERR, "Invalid packet format\n");
		goto write_failure;
	...
	return rc;
write_failure:
	if(packet_type == TSE_TAG_66_PACKET)
		rc = write_failure_packet(TSE_TAG_67_PACKET, reply);
	else
		rc = write_failure_packet(TSE_TAG_65_PACKET, reply);
	free(key);
	free(signature);
	free(key_out);
	if (auth_tok) {
