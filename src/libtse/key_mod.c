...
/**
 * tse_generate_sig_from_key_data
 * @sig:
 * @key_data:
 * @key_data_len:
 */
...
static int tse_dummy_init(char **alias)
{
...
static int tse_dummy_get_gen_key_params(struct key_mod_param **params,
					     uint32_t *num_params)
{
	if (tse_verbosity)
		syslog(LOG_INFO,
...
static int
tse_dummy_get_gen_key_subgraph_trans_node(struct transition_node **trans,
					       uint32_t version)
{
	if (tse_verbosity)
		syslog(LOG_INFO,
...
static int
tse_dummy_get_params(struct key_mod_param **params, uint32_t *num_params)
{
	if (tse_verbosity)
		syslog(LOG_INFO,
		       "%s: Dummy function substituted for unimplemented "
...
static int
tse_dummy_get_param_subgraph_trans_node(struct transition_node **trans,
					     uint32_t version)
{
	if (tse_verbosity)
		syslog(LOG_INFO,
...
static int tse_dummy_get_blob(unsigned char *blob, size_t *blob_size,
...
static int
tse_dummy_get_key_data(unsigned char *key_data, size_t *key_data_len,
			    unsigned char *blob)
{
	if (tse_verbosity)
		syslog(LOG_INFO,
...
static int
tse_dummy_get_key_sig(unsigned char *sig, unsigned char *blob)
{
	if (tse_verbosity)
		syslog(LOG_INFO,
		       "%s: Dummy function substituted for unimplemented "
...
static int tse_dummy_get_key_hint(unsigned char *hint, size_t *hint_len,
				       unsigned char *blob)
{
	if (tse_verbosity)
		syslog(LOG_WARNING,
...
static int
tse_dummy_encrypt(char *to, size_t *to_size, char *from, size_t from_size,
...
static int
tse_dummy_decrypt(char *to, size_t *to_size, char *from, size_t from_size,
		       unsigned char *blob, int blob_type)
{
	syslog(LOG_WARNING, "%s: Dummy function substituted for unimplemented "
	       "function in key module\n", __FUNCTION__);
...
static int tse_dummy_destroy(unsigned char *blob)
{
	if (tse_verbosity)		
		syslog(LOG_INFO,
		       "%s: Dummy function substituted for unimplemented "
...
static int tse_dummy_finalize(void)
{
	if (tse_verbosity)		
		syslog(LOG_INFO,
		       "%s: Dummy function substituted for unimplemented "
...
{
	if (!key_mod_ops->init)
		key_mod_ops->init = &tse_dummy_init;
	if (!key_mod_ops->get_gen_key_params)
		key_mod_ops->get_gen_key_params =
			&tse_dummy_get_gen_key_params;
	if (!key_mod_ops->get_gen_key_subgraph_trans_node)
		key_mod_ops->get_gen_key_subgraph_trans_node =
			&tse_dummy_get_gen_key_subgraph_trans_node;
	if (!key_mod_ops->get_params)
		key_mod_ops->get_params = &tse_dummy_get_params;
	if (!key_mod_ops->get_param_subgraph_trans_node)
		key_mod_ops->get_param_subgraph_trans_node =
			&tse_dummy_get_param_subgraph_trans_node;
	if (!key_mod_ops->get_blob)
		key_mod_ops->get_blob = &tse_dummy_get_blob;
	if (!key_mod_ops->get_key_data)
		key_mod_ops->get_key_data = &tse_dummy_get_key_data;
	if (!key_mod_ops->get_key_sig)
		key_mod_ops->get_key_sig = &tse_dummy_get_key_sig;
	if (!key_mod_ops->get_key_hint)
		key_mod_ops->get_key_hint = &tse_dummy_get_key_hint;
	if (!key_mod_ops->encrypt)
		key_mod_ops->encrypt = &tse_dummy_encrypt;
	if (!key_mod_ops->decrypt)
		key_mod_ops->decrypt = &tse_dummy_decrypt;
	if (!key_mod_ops->destroy)
		key_mod_ops->destroy = &tse_dummy_destroy;
	if (!key_mod_ops->finalize)
		key_mod_ops->finalize = &tse_dummy_finalize;
	return 0;
}

/**
 * Called from: src/libtse/module_mgr.c::tse_process_decision_graph
 */
int tse_register_key_modules(struct tse_ctx* ctx)
{
	...
	char *dir_name = NULL;
	int i;
	struct tse_key_mod *curr_key_mod = &(ctx->key_mod_list_head);
	struct tse_key_mod_ops *(*walker)(void);
	int rc = 0;

	if (asprintf(&dir_name, "%s", TSE_DEFAULT_KEY_MOD_DIR) == -1) {
		rc = -ENOMEM;
		goto out;
	}
	...
	while ((ep = readdir(dp))) {
		struct tse_key_mod *new_key_mod = NULL;
		size_t dir_length;
		char *path = NULL;
		char *key_mod_dir = TSE_DEFAULT_KEY_MOD_DIR;
		void *handle;
		...
		get_key_mod_ops = (struct tse_key_mod_ops *(*)(void))
		...
		new_key_mod = malloc(sizeof(struct tse_key_mod));
		if (!new_key_mod) {
			syslog(LOG_ERR, "Out of memory\n");
			closedir(dp);
			dlclose(handle);
		...
		memset(new_key_mod, 0, sizeof(struct tse_key_mod));
		...
		if ((rc = tse_fill_in_dummy_ops(new_key_mod->ops))) {
	...
	while (walker) {
		struct tse_key_mod *new_key_mod;
		struct tse_key_mod *tmp_key_mod;

		if (!(new_key_mod = malloc(sizeof(struct tse_key_mod)))) {
			syslog(LOG_ERR, "Out of memory\n");
			rc = -ENOMEM;
			goto out;
		}
...
{
	struct tse_key_mod *curr = ctx->key_mod_list_head.next;
	struct tse_key_mod *temp;
